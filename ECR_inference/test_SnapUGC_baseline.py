...
def sound_classification(path):
    if True:        
        out_path = path[0:-4]+".wav"
        cmd = "ffmpeg -v quiet -y -i %s -ac 1 -f wav %s"%(path, out_path)
        # print(cmd)
        os.system(cmd)
...
def main():
    ...
    # print("paramter", args.video_dir, args.csv_file)
    with torch.no_grad():
        calculate(args.videos_dir, args.csv_file)
